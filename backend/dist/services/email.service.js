...
export async function sendMail(input) {
    try {
        if (transport.kind === 'dev') {
            logger.info({ to: input.to, subject: input.subject, tags: input.tags }, '[email:dev] would send mail (no transport configured)');
            return true;
        }
        if (transport.kind === 'resend') {
            await sendViaResend(input);
            logger.info({ to: input.to, subject: input.subject, transport: 'resend' }, 'email sent');
            return true;
        }
        ...
        logger.error({ err, to: input.to, subject: input.subject, errorMessage: msg }, 'sendMail failed');
        return false;
    }
}
/**
...
export async function verifyTransport() {
    try {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        return { ok: false, kind: transport.kind, error: err?.message ?? String(err) };
    }
}
