...
function numberToIndianWords(n) {
    ...
    const parts = [];
    if (crore)
        parts.push(`${below100(crore)} Crore`);
    if (lakh)
        parts.push(`${below100(lakh)} Lakh`);
    if (thousand)
        parts.push(`${below100(thousand)} Thousand`);
    if (hundred)
...
async function renderToBuffer(record, cycle, employee, tenant, 
...
_ytd) {
    return new Promise((resolve, reject) => {
        ...
        const fieldGap = 18;
        const gridY = y + 54;
        const drawField = (label, value, x, yy, w = 200) => {
        ...
        drawField('Date of Joining', formatDateDDMMYYYY(employee.joiningDate), rightColX, gridY + 24);
        // Move "Pay Period" to a small tag at the top-right so the grid
        // stays clean.
        doc.fillColor(COLOR.muted).font('Helvetica').fontSize(7.5).text('PAY PERIOD', rightColX, y + 14, { characterSpacing: 0.7 });
        doc
        ...
        // Section headers
        const sectionH = 26;
        const drawSectionHeader = (text, x, color) => {
        ...
        const rowH = 22;
        const drawRow = (x, yy, label, amount, isTotal = false) => {
            if (isTotal) {
        ...
        if (record.bankDetails?.accountNumber) {
            ...
            const drawBankField = (label, value, x, w) => {
            ...
            drawBankField('Bank', record.bankDetails.bankName ?? '—', PAD + 18, 160);
            drawBankField('Account No.', maskAccount(record.bankDetails.accountNumber), PAD + 18 + 180, 140);
            drawBankField('IFSC', record.bankDetails.ifscCode ?? '—', PAD + 18 + 340, 120);
            y += bnkH + 14;
        }
        /* ────────────────────────────────────────────────────────
           8. FOOTER
        ...
        doc
            ...
            .font('Helvetica')
            .fontSize(8)
            .text('This is a computer-generated payslip and does not require a signature or stamp.', PAD, pageH - 44, { width: contentW, align: 'center' });
        doc
            .fillColor(COLOR.muted)
            .font('Helvetica-Oblique')
            .fontSize(7.5)
            .text(`Generated on ${new Date().toLocaleString('en-IN', { dateStyle: 'medium', timeStyle: 'short' })}`, PAD, pageH - 28, { width: contentW, align: 'center' });
        doc.end();
    });
}
export async function generatePayslipPdf(record, cycle, employee, tenant, ytd) {
