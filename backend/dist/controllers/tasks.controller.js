...
export async function listTasks(req, res) {
    ...
    if (q.milestoneId)
        filter.milestoneId = q.milestoneId;
...
export async function updateTask(req, res) {
    ...
    const update = { ...body };
    if (body.assignee)
        update.assignee = new Types.ObjectId(body.assignee);
...
export async function updateTaskStatus(req, res) {
    const body = req.body;
    const doc = await Task.findByIdAndUpdate(String(req.params.id), { status: body.status, ...(body.order !== undefined ? { order: body.order } : {}) }, { new: true }).exec();
    if (!doc)
        throw new NotFoundError('Task not found');
    res.json({ success: true, data: doc });
}
export async function reorderTasks(req, res) {
    const body = req.body;
    await Promise.all(body.items.map((it) => Task.findByIdAndUpdate(it._id, { status: it.status, order: it.order }).exec()));
