...
export async function assignShift(req, res) {
    ...
    if (!shift)
        throw new NotFoundError('Shift not found');
    const result = await Employee.updateMany({ _id: { $in: employeeIds } }, { $set: { shift: shift._id } });
...
async function buildRoster(employees, from, to) {
    ...
    for (const emp of employees) {
        ...
        for (const date of eachDay(from, to)) {
            ...
            if (!isWorkDay || att === 'weekend' || att === 'holiday')
                ...
                status = 'off';
            else if (att === 'present' || att === 'late' || att === 'half_day')
                status = 'confirmed';
            out.push({
...
export async function myRoster(req, res) {
...
export async function teamRoster(req, res) {
    const userId = getUserId();
    if (!userId)
        throw new ForbiddenError('Not authenticated');
    const q = req.query;
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const filter = me ? { reportingManager: me._id } : { status: 'active' };
    if (q.departmentId)
        filter.department = q.departmentId;
