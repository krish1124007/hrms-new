...
export async function list(req, res) {
    ...
    if (q.mandatory)
        filter.mandatory = q.mandatory === 'true';
    if (q.tag)
        filter.tags = q.tag;
    if (q.search) {
    ...
    const result = await HrPolicy.paginate(filter, {
        ...
        select: 'policyCode title category status summary currentVersion mandatory effectiveDate reviewDueDate tags acknowledgements createdAt updatedAt',
    });
...
export async function update(req, res) {
    ...
    if (contentChanged && policy.status === 'published') {
        ...
        if (v) {
            ...
            if (body.changeNotes !== undefined)
                v.changeNotes = body.changeNotes;
        }
        else {
    ...
    if (body.effectiveDate !== undefined)
        policy.effectiveDate = body.effectiveDate;
    if (body.reviewDueDate !== undefined)
        policy.reviewDueDate = body.reviewDueDate;
    if (body.mandatory !== undefined)
        policy.mandatory = body.mandatory;
...
export async function publish(req, res) {
    ...
    if (v) {
        ...
        if (body.effectiveDate)
            v.effectiveDate = body.effectiveDate;
        if (body.changeNotes !== undefined)
            v.changeNotes = body.changeNotes;
    }
...
export async function acknowledge(req, res) {
    ...
    policy.acknowledgements = policy.acknowledgements.filter((a) => String(a.employee) !== body.employee);
    ...
    await policy.save();
    await policy.populate('acknowledgements.employee', 'firstName lastName employeeId email profileImage');
    res.json({ success: true, data: policy });
}
/** GET /api/v1/hr-policies/:id/acknowledgements */
export async function listAcknowledgements(req, res) {
    const policy = await HrPolicy.findById(String(req.params.id))
        .populate('acknowledgements.employee', 'firstName lastName employeeId email profileImage')
        .exec();
    if (!policy)
...
export async function uploadAttachment(req, res) {
    ...
    if (!isPdf)
        throw new ValidationAppError('Only PDF files are allowed for policy attachments');
    const policy = await HrPolicy.findById(String(req.params.id)).exec();
    if (!policy)
        throw new NotFoundError('Policy not found');
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    policy.attachments[policy.attachments.length - 1].key = key;
    await policy.save();
    void audit({ action: 'update', entity: 'HrPolicy', entityId: String(policy._id) });
    res.status(201).json({ success: true, data: policy });
}
...
export async function deleteAttachment(req, res) {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const att = policy.attachments.find((a) => String(a._id) === attachmentId);
    if (!att)
