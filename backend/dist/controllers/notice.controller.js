...
export async function createNotice(req, res) {
    ...
    const targetFilter = { status: 'active' };
...
export async function updateNotice(req, res) {
    ...
    if (typeof body.title === 'string')
        update.title = sanitizeStrict(body.title);
    if (typeof body.content === 'string')
        update.content = sanitizeRich(body.content);
    if (body.departments) {
...
export async function acknowledgeNotice(req, res) {
    ...
    if (!notice)
        throw new NotFoundError('Notice not found');
    const alreadyAcked = notice.acknowledgements.some((a) => String(a.userId) === String(userId));
    if (!alreadyAcked) {
