...
export async function listPayments(req, res) {
    ...
    if (q.from || q.to) {
        filter.collectedAt = {};
        if (q.from)
            filter.collectedAt.$gte = q.from;
        if (q.to)
            filter.collectedAt.$lte = q.to;
    }
...
export async function myPayments(_req, res) {
...
export async function createPayment(req, res) {
    ...
    await Client.updateOne({ _id: body.clientId }, { $inc: { totalPayments: body.amount, outstandingAmount: -body.amount } }).exec();
...
export async function updatePayment(req, res) {
    const doc = await PaymentCollection.findByIdAndUpdate(String(req.params.id), req.body, { new: true }).exec();
    if (!doc)
        throw new NotFoundError('Payment not found');
    res.json({ success: true, data: doc });
}
...
export async function verifyPayment(req, res) {
    const userId = getUserId();
    const doc = await PaymentCollection.findByIdAndUpdate(String(req.params.id), { status: 'verified', verifiedBy: userId }, { new: true }).exec();
    if (!doc)
        throw new NotFoundError('Payment not found');
    res.json({ success: true, data: doc });
}
...
export async function outstandingReport(_req, res) {
...
//# sourceMappingURL=field-payments.controller.js.map