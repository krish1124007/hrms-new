...
export async function markAllRead(req, res) {
    if (!req.user)
        throw new UnauthorizedError();
    const r = await Notification.updateMany({ userId: req.user._id, isRead: false }, { isRead: true, readAt: new Date() }).exec();
...
export async function registerDevice(req, res) {
    if (!req.user)
        throw new UnauthorizedError();
    const body = req.body;
    const doc = await PushDevice.findOneAndUpdate({ token: body.token }, {
        userId: req.user._id,
        token: body.token,
