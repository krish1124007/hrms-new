...
import { NotFoundError, ValidationAppError, UnauthorizedError, ForbiddenError, } from '../lib/errors.js';
...
export async function listDocuments(req, res) {
    ...
    if (q.folder)
        filter.folder = normaliseFolder(q.folder);
    if (q.search)
...
export async function listFolders(req, res) {
    ...
    res.json({
        success: true,
        data: folders.filter(Boolean).sort(),
    });
}
/**
...
export async function updateDocument(req, res) {
    ...
    const update = { ...body };
    if (body.folder)
        update.folder = normaliseFolder(body.folder);
