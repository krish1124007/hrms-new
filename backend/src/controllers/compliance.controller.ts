...
export async function deleteMyAccount(req: Request, res: Response): Promise<void> {
  ...
  if (otherAdmins === 0) {
    throw new ForbiddenError(
      'You are the last active user. Invite another admin before deleting your account.',
    );
  }
...
export async function recordConsent(req: Request, res: Response): Promise<void> {
  ...
  res.json({ success: true, data: { consents: (user as unknown as { consents: unknown }).consents } });
}

export async function complianceStatus(req: Request, res: Response): Promise<void> {
  if (!req.user) throw new UnauthorizedError();
  ...
  res.json({
    ...
    data: {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      consents: (req.user as any).consents ?? null,
      history: recent,
    },
  });
}
