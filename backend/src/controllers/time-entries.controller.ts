...
export async function createTimeEntry(req: Request, res: Response): Promise<void> {
  const body = req.body as z.infer<typeof createTimeEntrySchema>;
  const projectId = String(req.params.projectId);
  const me = getUserId();
...
export async function updateTimeEntry(req: Request, res: Response): Promise<void> {
  const body = req.body as z.infer<typeof updateTimeEntrySchema>;
  const doc = await TimeEntry.findByIdAndUpdate(String(req.params.id), body, { new: true }).exec();
  if (!doc) throw new NotFoundError('Time entry not found');
...
export async function getTimeEntrySummary(req: Request, res: Response): Promise<void> {
  const projectId = String(req.params.projectId);
...
export async function myTimesheets(req: Request, res: Response): Promise<void> {
  const me = getUserId();
  if (!me) {
  ...
  const q = req.query as any;
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const filter: Record<string, any> = { userId: new Types.ObjectId(me) };
  if (q.from || q.to) {
    filter.date = {};
    if (q.from) filter.date.$gte = new Date(q.from);
    if (q.to) filter.date.$lte = new Date(q.to);
...
export async function weeklyTimesheet(req: Request, res: Response): Promise<void> {
  const me = getUserId();
  if (!me) {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const byProject = new Map<string, { project: any; perDay: Record<string, number>; total: number }>();
