...
export async function assignEmployees(req: Request, res: Response): Promise<void> {
  const { employeeIds } = req.body as z.infer<typeof assignEmployeesSchema>;
  const site = await AttendanceSite.findByIdAndUpdate(
...
export const createGeofenceSchema = z
  ...
  .refine(
    (v) =>
      (v.type === 'circle' && v.center && v.radius) ||
      (v.type === 'polygon' && v.coordinates && v.coordinates.length >= 3),
    { message: 'Circle requires center+radius; polygon requires >=3 coordinates' },
  );
...
export async function listGeofences(req: Request, res: Response): Promise<void> {
  const q = req.query as unknown as z.infer<typeof listSchema>;
  const filter: Record<string, unknown> = {};
  if (q.search) filter.name = { $regex: q.search, $options: 'i' };
...
export async function createGeofence(req: Request, res: Response): Promise<void> {
  const body = req.body as z.infer<typeof createGeofenceSchema>;
  const zone = await GeofenceZone.create(body);
  void audit({ action: 'create', entity: 'GeofenceZone', entityId: String(zone._id) });
  res.status(201).json({ success: true, data: zone });
}

export async function getGeofence(req: Request, res: Response): Promise<void> {
  const zone = await GeofenceZone.findById(String(req.params.id)).exec();
  if (!zone) throw new NotFoundError('Geofence not found');
  res.json({ success: true, data: zone });
}

export async function updateGeofence(req: Request, res: Response): Promise<void> {
  const body = req.body as z.infer<typeof updateGeofenceSchema>;
...
export async function deleteGeofence(req: Request, res: Response): Promise<void> {
  const zone = await GeofenceZone.findById(String(req.params.id)).exec();
  if (!zone) throw new NotFoundError('Geofence not found');
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  await (zone as any).softDelete();
...
export async function createQRCode(req: Request, res: Response): Promise<void> {
  const body = req.body as z.infer<typeof createQRSchema>;
...
export async function rotateDynamicQR(_req: Request, res: Response): Promise<void> {
...
export async function listAllowedIPs(req: Request, res: Response): Promise<void> {
  const q = req.query as unknown as z.infer<typeof listSchema>;
  const result = await AllowedIP.paginate({}, { page: q.page, limit: q.limit, sort: 'label' });
  res.json({ success: true, data: result.data, pagination: result.pagination });
}

export async function createAllowedIP(req: Request, res: Response): Promise<void> {
  const body = req.body as z.infer<typeof createAllowedIPSchema>;
...
export async function updateAllowedIP(req: Request, res: Response): Promise<void> {
  const body = req.body as z.infer<typeof updateAllowedIPSchema>;
  const ip = await AllowedIP.findByIdAndUpdate(String(req.params.id), body, { new: true }).exec();
  if (!ip) throw new NotFoundError('Allowed IP not found');
  void audit({ action: 'update', entity: 'AllowedIP', entityId: String(ip._id), after: body });
  res.json({ success: true, data: ip });
}

export async function deleteAllowedIP(req: Request, res: Response): Promise<void> {
  const ip = await AllowedIP.findById(String(req.params.id)).exec();
  if (!ip) throw new NotFoundError('Allowed IP not found');
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  await (ip as any).softDelete();
