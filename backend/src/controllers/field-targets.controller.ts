...
async function recomputeAchieved(target: { _id: unknown; employeeId: unknown; period: { month: number; year: number }; type: string; targetValue: number; achievedValue: number; status: string }): Promise<void> {
  ...
  await SalesTarget.updateOne(
    { _id: target._id },
    { $set: { achievedValue: achieved, status } },
  ).exec();
}
...
export async function listTargets(req: Request, res: Response): Promise<void> {
  ...
  if (q.month) filter['period.month'] = q.month;
  if (q.year) filter['period.year'] = q.year;
...
export async function myTargets(_req: Request, res: Response): Promise<void> {
...
export async function getTarget(req: Request, res: Response): Promise<void> {
  const doc = await SalesTarget.findById(String(req.params.id))
    ...
    .exec();
  if (!doc) throw new NotFoundError('Target not found');
