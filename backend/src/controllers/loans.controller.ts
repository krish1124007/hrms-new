...
import { z } from 'zod';
import { Loan, type ILoanInstallment, type LoanStatus } from '../models/loan.model.js';
...
export async function myLoans(_req: Request, res: Response): Promise<void> {
...
function buildSchedule(
  ...
  tenureMonths: number,
  startMonth: Date,
): { schedule: ILoanInstallment[]; totalInterest: number; totalPayable: number; emi: number } {
  const emi = computeEmi(principal, annualRatePct, tenureMonths);
  const monthlyRate = annualRatePct / 12 / 100;

  const schedule: ILoanInstallment[] = [];
...
export async function loanStats(_req: Request, res: Response): Promise<void> {
  ...
  for (const s of LOAN_STATUSES) statusCounts[s] = 0;
...
export async function createLoan(req: Request, res: Response): Promise<void> {
  ...
  const { schedule, totalInterest, totalPayable, emi } = buildSchedule(
...
export async function updateLoan(req: Request, res: Response): Promise<void> {
  const body = req.body as z.infer<typeof updateLoanSchema>;
  ...
  if (body.type !== undefined) loan.type = body.type;
  if (body.reason !== undefined) loan.reason = body.reason;
...
export async function rejectLoan(req: Request, res: Response): Promise<void> {
  const body = req.body as z.infer<typeof rejectLoanSchema>;
  const loan = await Loan.findById(String(req.params.id)).exec();
  if (!loan) throw new NotFoundError('Loan not found');
  if (loan.status !== 'pending') {
...
export async function disburseLoan(req: Request, res: Response): Promise<void> {
  const body = req.body as z.infer<typeof disburseLoanSchema>;
  const loan = await Loan.findById(String(req.params.id)).exec();
  if (!loan) throw new NotFoundError('Loan not found');
...
export async function recordPayment(req: Request, res: Response): Promise<void> {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const inst = loan.installments.find((i) => String((i as any)._id) === body.installmentId);
  ...
  if (body.amount > remaining + 0.01) {
    throw new ValidationAppError(
      `Payment exceeds remaining installment amount (₹${remaining.toFixed(2)})`,
    );
  }

  inst.paidAmount = round2(inst.paidAmount + body.amount);
  inst.paidOn = body.paidOn ?? new Date();
  if (body.notes) inst.notes = body.notes;
  ...
  const principalShare =
    inst.amount > 0 ? (inst.principalAmount / inst.amount) * body.amount : body.amount;
...
export async function previewEmi(req: Request, res: Response): Promise<void> {
  const body = req.body as z.infer<typeof previewEmiSchema>;
