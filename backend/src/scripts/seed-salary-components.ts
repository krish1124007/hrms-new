...
async function run(): Promise<void> {
  ...
  for (const c of COMPONENTS) {
    const updated = await SalaryComponent.findOneAndUpdate(
      { code: c.code },
      {
        ...c,
        isTaxable: false,
  ...
  const components = [
    { componentId: ids.BASIC, calculationType: 'percentage_of_gross' as const, value: 50 },
    { componentId: ids.HRA, calculationType: 'percentage_of_gross' as const, value: 30 },
    { componentId: ids.OTHER_ALL, calculationType: 'percentage_of_gross' as const, value: 20 },
    { componentId: ids.EMP_PF, calculationType: 'percentage_of_basic' as const, value: 12 },
    { componentId: ids.PT, calculationType: 'fixed' as const, value: 200 },
    { componentId: ids.EMPR_PF, calculationType: 'percentage_of_basic' as const, value: 13 },
  ];
