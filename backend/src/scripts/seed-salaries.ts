...
async function run(): Promise<void> {
  ...
  const rows = JSON.parse(raw) as SalaryRow[];
  ...
  for (const r of rows) {
    ...
    emp.salary = {
      ...
      otherAllowances: {},
      grossSalary: r.gross,
    } as typeof emp.salary;
    await emp.save();
    updated += 1;
    logger.info(
      `  ${r.employeeId}  ${r.fullName.padEnd(28)} ` +
  ...
  if (missing.length) logger.warn({ missing }, 'employees not found — skipped');
