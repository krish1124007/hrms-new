...
router.get(
  ...
  asyncHandler(ctrl.listAssets),
);
router.get('/stats', requirePermission('assets.view'), asyncHandler(ctrl.assetStats));
router.post(
  '/',
  requirePermission('assets.manage'),
  validate(ctrl.createAssetSchema),
  asyncHandler(ctrl.createAsset),
);
router.get('/:id', requirePermission('assets.view'), asyncHandler(ctrl.getAsset));
router.patch(
  '/:id',
  requirePermission('assets.manage'),
  validate(ctrl.updateAssetSchema),
  asyncHandler(ctrl.updateAsset),
);
router.delete('/:id', requirePermission('assets.manage'), asyncHandler(ctrl.deleteAsset));
router.post(
  '/:id/assign',
  requirePermission('assets.manage'),
  validate(ctrl.assignSchema),
  asyncHandler(ctrl.assignAsset),
);
router.post(
  '/:id/unassign',
  requirePermission('assets.manage'),
  validate(ctrl.unassignSchema),
  asyncHandler(ctrl.unassignAsset),
);
