...
async function run(): Promise<void> {
  ...
  if (!emp) throw new Error('Khyati not found');
  ...
  const jan2026 = (d: number): Date => new Date(2026, 0, d);
  ...
  for (const d of sundays) status[d] = 'weekend';
  for (const d of holidays) status[d] = 'holiday';

  // Pick 2 unpaid + 2 paid leave + 2 late from the remaining weekdays
  const remainingDays = Array.from({ length: 31 }, (_, i) => i + 1).filter(
    (d) => !status[d],
  );
  ...
  for (const d of unpaid) status[d] = 'absent';
  for (const d of paidLeave) status[d] = 'on_leave';
  for (const d of late) status[d] = 'late';
  for (const d of present) status[d] = 'present';
