...
export async function getCategory(req: Request, res: Response): Promise<void> {
  ...
  if (!doc) throw new NotFoundError('Expense category not found');
...
export async function listClaims(req: Request, res: Response): Promise<void> {
  ...
  if (q.category) filter.category = new Types.ObjectId(q.category);
  if (q.from || q.to) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
...
export async function myClaims(req: Request, res: Response): Promise<void> {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const q = req.query as any;
  const result = await ExpenseClaim.paginate(
...
export async function teamClaims(req: Request, res: Response): Promise<void> {
...
export async function getClaim(req: Request, res: Response): Promise<void> {
  const doc = await ExpenseClaim.findById(String(req.params.id))
    ...
    .exec();
  if (!doc) throw new NotFoundError('Expense claim not found');
  res.json({ success: true, data: doc });
}
...
export async function rejectClaim(req: Request, res: Response): Promise<void> {
  const body = req.body as z.infer<typeof rejectClaimSchema>;
  const doc = await ExpenseClaim.findById(String(req.params.id)).exec();
  if (!doc) throw new NotFoundError('Expense claim not found');
  if (doc.status !== 'pending') {
...
export async function reimburseClaim(req: Request, res: Response): Promise<void> {
  const body = req.body as z.infer<typeof reimburseClaimSchema>;
  const doc = await ExpenseClaim.findById(String(req.params.id)).exec();
  if (!doc) throw new NotFoundError('Expense claim not found');
...
export async function expenseReports(req: Request, res: Response): Promise<void> {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const q = req.query as any;
