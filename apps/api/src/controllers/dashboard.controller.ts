...
export async function attendanceTrend(_req: Request, res: Response): Promise<void> {
  const days = 7;
  ...
  const leavesByDay = await LeaveRequest.aggregate<{ startDate: Date; endDate: Date }>([
    {
      $match: {
        status: 'approved',
        startDate: { $lte: to },
...
export async function analytics(_req: Request, res: Response): Promise<void> {
  ...
  const revenueTrend = Array.from({ length: 6 }, (_, i) => {
    ...
    const row = revenueByMonth.find(
      (r) => r._id.y === d.getFullYear() && r._id.m === d.getMonth() + 1,
    );
    return {
  ...
  const headcountAgg = await Employee.aggregate<{ _id: unknown; count: number }>([
  ...
  const leavesAgg = await LeaveRequest.aggregate<{ _id: unknown; count: number }>([
    {
      $match: {
        status: 'approved',
  ...
  const topPerformers = topTargets
    ...
    .map((t) => {
      const emp = t.employeeId as unknown as { firstName?: string; lastName?: string; designation?: string };
      return {
  ...
  const lagDays =
    recentApproved.length > 0
      ? recentApproved.reduce((sum, r) => {
          ...
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          const updated = new Date((r as any).updatedAt).getTime();
