...
router.post(
  '/',
  requirePermission('disciplinary.manage'),
...
router.post(
  '/:id/acknowledge',
  requirePermission('disciplinary.view'),
  validate(ctrl.acknowledgeSchema),
  asyncHandler(ctrl.acknowledge),
);
router.post(
  '/:id/resolve',
  requirePermission('disciplinary.manage'),
  validate(ctrl.resolveSchema),
  asyncHandler(ctrl.resolve),
);
router.post(
  '/:id/escalate',
  requirePermission('disciplinary.manage'),
  validate(ctrl.escalateSchema),
  asyncHandler(ctrl.escalate),
);
router.post(
  '/:id/cancel',
  requirePermission('disciplinary.manage'),
  validate(ctrl.cancelSchema),
...
router.post(
  '/:id/comments',
  requirePermission('disciplinary.manage'),
  validate(ctrl.addCommentSchema),
  asyncHandler(ctrl.addComment),
);
router.post(
  '/:id/attachments',
  requirePermission('disciplinary.manage'),
  validate(ctrl.addAttachmentSchema),
  asyncHandler(ctrl.addAttachment),
);
router.delete(
  '/:id/attachments/:attId',
  requirePermission('disciplinary.manage'),
  asyncHandler(ctrl.removeAttachment),
);
