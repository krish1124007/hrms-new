...
const perRequest = new Map<string, Map<string, number>>();
 ...
 */
export function recordQuery(collection: string): void {
  const ctx = getContext();
  const reqId = ctx?.requestId;
  if (!reqId) return; // outside request scope (BullMQ job, CLI, startup)
  const perColl = perRequest.get(reqId) ?? new Map<string, number>();
...
export function _counts(requestId: string): Record<string, number> | null {
