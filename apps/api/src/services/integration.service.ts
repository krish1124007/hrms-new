...
export async function onEmployeeCreated(
  ...
  try {
    ...
    if (existingUser) {
      ...
      logger.info(
        { employeeId: employee._id, userId: existingUser._id },
        'Linked existing user to employee',
      );
      return;
    }
    ...
    logger.info(
      { employeeId: employee._id, userId: user._id },
      'Auto-created user for new employee',
    );
  } catch (err) {
 ...
 */
export async function onEmployeeDeactivated(
  employeeId: string,
  ): Promise<void> {
  try {
    ...
    logger.info(
      { employeeId, userId: employee.userId },
      'Deactivated user and cleared sessions for employee',
    );
  } catch (err) {
...
export async function getAttendanceSummaryForPayroll(
  ...
  const approvedLeaves = await LeaveRequest.find({
    ...
    .populate<{ leaveTypeId: { paidLeave: boolean } }>('leaveTypeId', 'paidLeave')
    .lean();
...
export interface LeaveDeductions {
 ...
 */
export async function getLeaveDeductionsForPayroll(
  employeeId: string,
  month: number,
  year: number,
): Promise<LeaveDeductions> {
  const startDate = new Date(year, month - 1, 1);
  const endDate = new Date(year, month, 0, 23, 59, 59, 999);
...
export interface LoanEMI {
  totalEMI: number;
  loans: Array<{ loanId: string; emiAmount: number; remaining: number }>;
}
 ...
 */
export async function getLoanEMIForPayroll(
  _employeeId: string,
  _month: number,
  _year: number,
): Promise<LoanEMI> {
