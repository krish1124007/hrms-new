...
interface BackupCreateJob {
  backupId: string;
}

interface BackupRestoreJob {
  backupId: string;
  /** Where to restore to. MUST point at a scratch DB, never prod. */
  scratchMongoUri: string;
}

/* ─────────── Create ─────────── */

async function runBackup(job: Job<BackupCreateJob>): Promise<void> {
  ...
  try {
    ...
    row.status = 'failed';
    row.error = (err as Error).message.slice(0, 1000);
    row.completedAt = new Date();
    await row.save();
    throw err;
  } finally {
...
async function runRestore(job: Job<BackupRestoreJob>): Promise<void> {
  ...
  if (
    scratchMongoUri === env.MONGODB_URI ||
  ...
  try {
    ...
    execSync(
      `mongorestore --uri="${scratchMongoUri}" --drop --quiet ${path.join(scratch, 'dump')}`,
      { stdio: 'pipe' },
    );
...
function isLastDayOfMonth(d: Date = new Date()): boolean {
  const tomorrow = new Date(d);
  tomorrow.setDate(d.getDate() + 1);
  return tomorrow.getMonth() !== d.getMonth();
}

async function runScheduledMonthly(): Promise<void> {
  if (!isLastDayOfMonth()) {
  ...
  logger.info(
    ...
    'backup-scheduled-monthly: row created, running mongodump',
  );
  await runBackup({ data: { backupId: String(backup._id) } } as Job<BackupCreateJob>);
}

/** Returns whether the monthly schedule is registered with BullMQ. */
export async function getMonthlyBackupSchedule(): Promise<{
  enabled: boolean;
  pattern: string;
  description: string;
  nextRun?: number;
 ...
 */
export async function setMonthlyBackupSchedule(enabled: boolean): Promise<void> {
  const repeatable = await backupQueue.getRepeatableJobs();
...
export function startBackupWorker(): Worker {
  const worker = new Worker(
    QUEUE_NAME,
    async (job: Job) => {
      if (job.name === 'backup-create') return runBackup(job as Job<BackupCreateJob>);
      if (job.name === 'backup-restore') return runRestore(job as Job<BackupRestoreJob>);
      if (job.name === SCHEDULED_JOB_NAME) return runScheduledMonthly();
      logger.warn({ name: job.name }, 'backup: unknown job');
    },
    { connection: redis, concurrency: 1 }, // one at a time — backups are I/O-heavy
  );
  worker.on('failed', (job, err) =>
    logger.error({ jobName: job?.name, err }, 'backup worker job failed'),
  );
  return worker;
}
...
export async function enqueueBackupCreate(
  backupId: string,
  ): Promise<void> {
  await backupQueue.add('backup-create', { backupId });
}

export async function enqueueBackupRestore(
  backupId: string,
  scratchMongoUri: string,
): Promise<void> {
